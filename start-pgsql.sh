...
if [[ ! -f /opt/postgresql/initialized ]]; then
    ...
    su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres --single  -D  /var/lib/postgresql/9.3/main  -c config_file=/etc/postgresql/9.3/main/postgresql.conf" <<< "CREATE DATABASE db WITH ENCODING 'UTF8' LC_COLLATE 'pl_PL.UTF-8' LC_CTYPE 'pl_PL.UTF-8' OWNER root TEMPLATE template0 ;"
